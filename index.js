  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (texto === "1") {
      ...
      if (!lavagemAtiva || lavagemAtiva.numero !== remetente) {
        await enviar({
          text: `⚠️ Nenhuma lavagem ativa ou você não está usando.`,
        });
        return;
      }
      ...
      if (lavagemAtiva && lavagemAtiva.numero !== remetente) {
        ...
        const restanteStr =
          minutosRestantes <= 0
            ...
            : `faltam aproximadamente ${Math.floor(
                duracaoRestante.asHours()
              )}h ${duracaoRestante.minutes()}min para finalizar.`;
    ...
    //Parte 9 — Recursos adicionais: sorteio, previsão do tempo, lixo
      ...
      let mensagem = `🧺 *Exemplo de carga ideal para lavagem (até 8kg):*\n\n${lista.join(
        "\n"
      )}\n\n⚖️ Peso total estimado: *${totalKg.toFixed(
        2
      ...
      try {
        const { data } = await axios.get(
          "https://api.hgbrasil.com/weather?key=c657e670&city_name=Viamão,RS"
        );
        ...
        const horaUltimaAtualizacao = `${dataUltimaAtualizacao.getHours()}:${dataUltimaAtualizacao
          .getMinutes()
          .toString()
          .padStart(2, "0")}`;
        ...
        await enviar({
          text: "❌ Não foi possível obter a previsão do tempo. Tente novamente mais tarde.",
        });
      }
    } else if (texto === "10") {
      // Mensagem de coleta de lixo mais envolvente
      const mensagem =
        `🗑️ **Dias de Coleta de Lixo em Viamão**\n\n` +
