...
if (texto === "1") {
  ...
  await enviar({ text: `🧺 Lavagem iniciada às ${formatarHorario(agora.toDate())}.\n⏱️ Termina às ${formatarHorario(fim.toDate())}` });
  setTimeout(async () => {
    await enviar({
  ...
  // adicionar lógica da fila aqui, se quiser
}
...
//Parte 9 — Recursos adicionais: sorteio, previsão do tempo, lixo
  ...
  await enviar({ text: "🎲 Sorteio de roupas não implementado ainda." });
} else if (texto === "8") {
  ...
  try {
    ...
    await enviar({ text: `🌦️ Clima em Viamão: ${info.temp}°C\n💧 Umidade: ${info.humidity}%` });
  } catch {
