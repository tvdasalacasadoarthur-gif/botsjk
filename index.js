const {
  ...
  DisconnectReason, // 👈 adicione isso aqui
...
async function iniciar() {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (texto === "1") {
      ...
      await sock.sendMessage(remetente, {
        text: `🧺 Lavagem iniciada às ${formatarHorario(
          agora.toDate()
        )}.\n⏱️ Finaliza às ${formatarHorario(
          fim.toDate()
      ...
      setTimeout(async () => {
        await sock.sendMessage(remetente, {
          text: `🔔 @${
            remetente.split("@")[0]
          } sua lavagem vai finalizar em 5 minutos.`,
          mentions: [remetente],
        });
      }, 1.55 * 60 * 60 * 1000);
      ...
      const posicao = global.usuariosNaFila.findIndex(
        (u) => u.numero === remetente
      );
      if (posicao === -1) {
        global.usuariosNaFila.push({
          nome: nomeUsuario,
          numero: remetente,
          hora: new Date(),
        });
      ...
      combinacao.forEach(
        (r) => (resposta += `• ${r.nome} (${r.peso.toFixed(2)}kg)\n`)
      );
